...
Settings={}
Settings.Name = "Market Breadth"
Settings.SecCode = "None"
Settings.ClassCode = "SPBFUT"
Settings.Ratio = 0 -- Если Settings.Ratio = 0, то вычисляется отношение цен инструментов Related/Base, в противном случае вычисляется
                   -- разность цен инструментов Related - (Base * Settings.Ratio). Settings.Ratio - может быть отрицательным числом
...
function OnCalculate(index)
	...
	if isInit then
		PrintDbgStr("Получаем источник данных графика")
		SourceInfo = getDataSourceInfo() -- получаем информацию об источнике данных графика
		NumberOf = getNumberOf("all_trades") -- получаем число строк в таблице обезличенных сделок
		PrintDbgStr("NumberOf: " .. NumberOf)
		local indexes = SearchItems("all_trades", 0, NumberOf - 1, TradesFilter, "sec_code, class_code") -- получаем индексы сделок заданного (related) инструмента
		if #indexes == 0 then -- такая ситуация возникнет, если в таблице обезличенных сделок нет сделок по "связанному" инструменту
			PrintDbgStr("Не могу получить сделки связанного инструмента")
			error("Не могу получить сделки связанного инструмента")
			return value
		end
...
function GetSecurities()
	...
	local classes = split(getClassesList(), ",")
	local codes = {}
	
	for class in classes do
		local cds = split(getClassSecurities(class), ",")
		for code in cds do
			codes[code].Class = class
		end
	end	
	
	path = getScriptPath() .. "\\MICEXINDEXCF.csv"
	
	local securities = {}
	
	for line in io.lines(path) do 
	    
		PrintDbgStr("line: " .. line)
		
		if string.find(line, "MICEXINDEXCF") ~= nil  then
			
			local values = split(line, ";")
			local security = {}
			security.Code = values[3] 
			security.WF = values[10] 
			security.Weight = values[13] 
			security.Class = codes[security.Code].Class
			
			securities[security.Code] = security 
			
			PrintDbgStr("Code: " .. security.Code .. " WF: " .. security.WF .. " Weight: " .. security.Weight)
...
function GetClassByCode(code)
	if code == "AFKS" then
	elseif code == "AFLT" then
	elseif code == "AGRO" then
	elseif code == "AKRN" then
	elseif code == "ALRS" then
	elseif code == "BANE" then
	elseif code == "BANEP" then
	elseif code == "CBOM" then
	elseif code == "CHMF" then
	elseif code == "DIXY" then
	elseif code == "FEES" then
	elseif code == "GAZP" then
	elseif code == "GMKN" then
	elseif code == "HYDR" then
	elseif code == "IRAO" then
	elseif code == "LKOH" then
	elseif code == "LSRG" then
	elseif code == "MAGN" then
	elseif code == "MFON" then
	elseif code == "MGNT" then
	elseif code == "MOEX" then
	elseif code == "MSNG" then
	elseif code == "MTLR" then
	elseif code == "MTSS" then
	elseif code == "MVID" then
	elseif code == "NLMK" then
	elseif code == "NMTP" then
	elseif code == "NVTK" then
	elseif code == "PHOR" then
	elseif code == "PIKK" then
	elseif code == "PLZL" then
	elseif code == "POLY" then
	elseif code == "ROSN" then
	elseif code == "RSTI" then
	elseif code == "RTKM" then
	elseif code == "RTKMP" then
	elseif code == "RUAL" then
	elseif code == "SBER" then
	elseif code == "SBERP" then
	elseif code == "SNGS" then
	elseif code == "SNGSP" then
	elseif code == "TATN" then
	elseif code == "TATNP" then
	elseif code == "TRNFP" then
	elseif code == "UPRO" then
	elseif code == "URKA" then
	elseif code == "UWGN" then
	elseif code == "VSMO" then
	elseif code == "VTBR" then
	elseif code == "YNDX" then
	end
end
...
-- возвращaет таблицу со сплитованными значениями.. Индексация с 1
function split(s, pat)  

	local t = {} local i = 0 local j = 1 local b = 1    
	while true do
		i = string.find(s, pat, i + 1)
		if i == nil then break end
		t[j] = string.sub(s, b, i - 1)
		b = i + 1
		j = j + 1
	end
	return t
...
--1<column name="indexid"
--2<column name="tradedate"
--3<column name="ticker"
--4<column name="shortnames"
--5<column name="secids"
--6<column name="waprice"
--7<column name="issue_size_total"
--8<column name="cap_total"
--9<column name="ff_factor"
--10<column name="w_factor"
--11<column name="issue_size_index"
--12<column name="cap_index"
--13<column name="weight"
--14<column name="value"
--15<column name="num_trades"
--16<column name="volatility"
--17<column name="factora"
--18<column name="factorb"
--19<column name="influence"
--20<column name="determinat"
--21<column name="currencyid"
